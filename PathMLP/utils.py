...
class ModelTrainer:
    ...
    def train_single_epoch(self):
        ...
        for b, batch in enumerate(self.train_loader):
            ...
            if data_format == 'cmu':
                ...
                agent_masks_idx = torch.arange(
                    len(future_agent_masks), device=self.device)[mask_comb]
                agent_tgt_three_mask[agent_masks_idx] = True
            ...
            if self.beta != 0.0:
                if self.ploss_type == 'mseloss':
                    ...
                    ploss = self.ploss_criterion(
                        gen_goals, future_agents_traj[:, -1, :].unsqueeze(1))
                else:
                    ...
                    ploss = self.ploss_criterion(
                        episode_idx, gen_goals, log_prior, -15.0)
            ...
            if self.beta != 0.0:
                if self.path_ploss_type == 'mseloss':
                    # ploss = self.ploss_criterion(gen_trajs, past_agents_traj)
                    ploss = self.path_ploss_criterion(
                        gen_trajs, future_agents_traj[:, :-1, :])
                else:
                    ploss = self.path_ploss_criterion(
            ...
            for candi in range(gen_goals.size(1)):
                ...
                gen_trajs = torch.cat(
                    (gen_trajs, gen_goals[:, candi].unsqueeze(1)), dim=2)
                gen_trajs_list.append(gen_trajs)

            gen_trajs = torch.stack(gen_trajs_list, dim=1).squeeze(2)

            rs_error3 = ((gen_trajs - future_agents_traj.unsqueeze(1)) **
                         2).sum(dim=-1).sqrt_()  # A X candi X T X 2 >> A X candi X T
    ...
    def inference(self):
        ...
        with torch.no_grad():
            ...
            for b, batch in enumerate(self.valid_loader):
                ...
                # A X candi X T X 2 >> A X candi X T
                rs_error3 = (
    ...
    def load_checkpoint(self, ckpt):
        ...
        self.goal_model.load_state_dict(
            checkpoint['path_model_state'], strict=False)
        self.path_model.load_state_dict(
            checkpoint['goal_model_state'], strict=False)
...
class ModelTest:
    def __init__(self, goal_model, path_model, data_loader, args, device, ploss_criterion=None):
        ...
        # self.test_ckpt = args.test_ckpt.split(
        #     '/')[-2] + "_" + args.test_ckpt.split('/')[-1]
                ...
                for b, batch in enumerate(self.data_loader):
                    ...
                    if self.beta != 0.0:
                        if self.ploss_type == 'mseloss':
                            # ploss = self.ploss_criterion(gen_trajs, past_agents_traj)
                            ploss = self.ploss_criterion(
                                gen_goals, tgt_trajs[:, -1, :].unsqueeze(1))
                        else:
                            ploss = self.ploss_criterion(
                                episode_idx, gen_goals, log_prior, -15.0)
                    ...
                    # A X candi X T X 2 >> A X candi X T
                    rs_error3 = ((gen_goals.squeeze(
                        2) - tgt_trajs[:, -1, :].unsqueeze(1)) ** 2).sum(dim=-1).sqrt_()
                    ...
                    for i in range(batch_size):
                        ...
                        dao_i, dao_mask_i = self.dao(
                            candidate_i, map_file_i)
                        dac_i, dac_mask_i = self.dac(
                            candidate_i, map_file_i)
    ...
    def vo_angle(gen_trajs, tgt_trajs, tgt_lens, ):
        ...
        for k in range(num_candidates):
            ...
            for i in range(num_tgt_agents):
                ...
                tgt_vectors = (np.concatenate(
                    (tgt_vector_x, tgt_vector_y), axis=1))
                gen_vectors = (np.concatenate(
                    (gen_vector_x, gen_vector_y), axis=1))
    ...
    def self_distance(gen_trajs, tgt_trajs, tgt_lens, src_trajs):
        ...
        for k in range(num_candidates):
            ...
            for i in range(num_tgt_agents):
                ...
                candidate_distance.append(
                    np.sum(np.linalg.norm(gen_traj_ki[:-1] - gen_traj_ki[1:])))
        ...
        for i in range(num_tgt_agents):
            ...
            for k in range(num_candidates-1):
                curr_candidate = np.tile(
                    curr_agent[k], (num_candidates-k-1, 1))
                min_temp_sd = np.min(np.sqrt(np.sum(np.power(
                    curr_candidate - curr_agent[k+1:], 2), axis=1))/(distance[k, i] + distance[k+1:, i]))
                max_temp_sd = np.max(np.sqrt(np.sum(np.power(
                    curr_candidate - curr_agent[k+1:], 2), axis=1))/(distance[k, i] + distance[k+1:, i]))
