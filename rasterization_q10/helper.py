...
class PredictHelper:
    ...
    def get_now_for_agent(self, instance_token: str, sample_token: str,
                          in_agent_frame: bool) -> Union[List[Record], np.ndarray]:

        starting_annotation = self.get_sample_annotation(instance_token, sample_token)

        coord = np.array(starting_annotation['translation'][:2])

        if in_agent_frame:
            coord = convert_global_coords_to_local(coord,
                                                   starting_annotation['translation'],
                                                   starting_annotation['rotation'])
